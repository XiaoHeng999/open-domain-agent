...
系统 SHALL 提供 `exec` 工具，通过异步子进程执行 shell 命令。支持超时控制和输出截断。

#### Scenario: 成功执行
- **WHEN** LLM 调用 `exec` 参数 `{"command": "python -c 'print(2+2)'"}`
- **THEN** 工具异步执行命令并返回 `"4\n"`
...
系统 SHALL 在执行 shell 命令前通过 SafetyManager 进行命令安全检查。
...
### Requirement: ExecTool 异步执行
系统 SHALL 使用 `asyncio.create_subprocess_shell()` 执行命令，不阻塞事件循环。

#### Scenario: 并发工具调用
- **WHEN** Agent 同时发起多个 `exec` 调用
- **THEN** 所有命令并发执行，互不阻塞（由 ReAct 循环顺序保证，但底层为异步）
